...
parser.add_argument('--lr_global', type=float, default=0.0001, help='Learning Rate Decrease Ratio per a Epoch')
...
# DATA Loader
save_dir = './result_origin/'  # save cropped image numpy array in pickle file
...
for video in train_video_files:
    ...
    original = train_metadata.loc[os.path.basename(video), 'original']
    if answer == 'REAL':
        real_list.append(os.path.basename(video))
    else:
        fake_list.append((os.path.basename(video), original))
...
while (i < len(real_list)):
    ...
    for iteration in range(0, num_frames + init_frames):
        ...
        if opt.crop:
            ...
            if real_file.split(".")[0] + ".pkl" in os.listdir(save_dir + "real_dict"):
                print(str(iteration), real_file, "real_file in save_dir")
                real_file_name = os.path.join(save_dir + 'real_dict', real_file.split('.')[0]) + '.pkl'
                with open(real_file_name, 'rb') as f:
                    file = list(pickle.load(f).values())[0]
                    print(len(file))
                    if len(file) != num_frames + init_frames - 1:
                        print("....but should do again cuz this file sucks")

                    else:
                        load_real_flag = True
                        if iteration == 0:
                            save_real_list.append(real_face_box)
                        else:
                            real_face_loc = file[iteration - 1]
                            real_face = face_detector_1.crop(real_frame, real_face_loc)
                            real_frame = torch.from_numpy(real_face)
            ...
            if not load_real_flag:
                real_face_loc, real_face, real_detect_acc, real_detect_failed, real_center_loc, real_face_moved = face_detector_1.crop_face(
                    detector, real_frame, real_face_box, h_size, v_size, real_width, real_height, ALPHA=0.5,
                    run_mode=run_mode)

                if real_face.any() == None:
                    if len(real_loc_list) == 0:
                        real_face_loc = real_face_box
                    else:
                        real_face_loc = real_loc_list[-1]
                    real_face = face_detector_1.crop(real_frame, real_face_loc)
                    real_frame = torch.from_numpy(real_face)
                    save_real_list.append(real_frame)
...
for epoch in range(opt.nEpochs):
    ...
    while (j < len(fake_list)):
        ...
        fake_name, fake_original = fake_file[0], fake_file[1]

        fake_capture = cv2.VideoCapture(train_dir + fake_name)
        save_fake_list = []
        ...
        for iteration in range(0, num_frames + init_frames):
            ...
            if opt.crop:
                ...
                real_file_name = os.path.join(save_dir + 'real', fake_original.split('.')[0]) + '.pkl'
                with open(real_file_name, 'rb') as f:
                    real_face = pickle.load(f)[iteration]

                if fake_name.split(".")[0] + ".pkl" in os.listdir(save_dir + 'fake'):
                    load_fake_flag = True
                    print(str(iteration), fake_name, "fake_file in save_dir")
                    fake_file_name = os.path.join(save_dir + 'fake', fake_name.split('.')[0]) + '.pkl'
                    with open(fake_file_name, 'rb') as f:
                        fake_face = pickle.load(f)[iteration]

            if not load_fake_flag:
                try:
                    fake_loc = real_loc_dict[fake_original][iteration]
                except KeyError:
                    NO_REAL_FILE = True
                    break
                fake_face = face_detector_1.crop(fake_frame, fake_loc)
                save_fake_list.append(fake_face)

                if iteration == (num_frames + init_frames - 1):
                    fake_save_dir = os.path.join(save_dir, "./fake/")
                    fake_file_name = os.path.join(fake_save_dir, fake_name.split('.')[0]) + '.pkl'
            ...
            if iteration % 10 == 1:
                print(
                    "===> Video_name:({},{}), Video_idx:{:3d}/{:3d}, ({:4d}/{:4d})".format(
                        real_file, fake_file, j + 1,
                        len(fake_list), iteration + 1, num_frames))

            if run_mode == 1:
                real_frame = cv2.resize(real_face, (h_size, v_size), interpolation=cv2.INTER_LINEAR)
                fake_frame = cv2.resize(fake_face, (h_size, v_size), interpolation=cv2.INTER_LINEAR)
        ...
        # SAVE MODEL
        if opt.save:
            model_out_path = "pretrain/0225_frame_model_epoch_{}.pth".format(epoch)
            torch.save(model, model_out_path)
            print("Checkpoint saved to {}".format(model_out_path))

            model_out_path2 = "pretrain/0225_global_model_epoch_{}.pth".format(epoch)
